function(video.folders,save.csv=F, output.folder,return.empty=F) {
  
   
 video.folders<-list.dirs(video.folders, full.names=TRUE,recursive = F)
 
 
dfMaker2<<- function(video.folders,output.folder,save.csv,return.empty) {
  ...
  dfMaker<<-function(file){
    ...
    triplet<-c(T,F,F)
    ###
    groups <- c("x", "y", "u") # the variables of the final df
    if(!is.null(dfPoints)){
 ...
 for (i in 1:(length(video.folders))) {
  ...
  dfVideo<-dfMaker2( video.folders[i],save.csv = save.csv,output.folder = output.folder,return.empty = return.empty )
